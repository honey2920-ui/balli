...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const { role, coreId, logout } = useAppStore();
  ...
  return (
    <div className="min-h-screen pb-24 relative overflow-hidden font-sans">
      <div id="falling-container" className="fixed inset-0 pointer-events-none z-0 opacity-[0.03]" />
      <DynamicIsland />
      <LoginModal />

      <div className="p-5 flex justify-between items-center relative z-10 max-w-4xl mx-auto">
      ...
      {role && (
        <nav className="fixed bottom-0 left-0 w-full h-[84px] bg-[#12163f]/90 backdrop-blur-xl border-t border-white/10 flex justify-around items-center z-50 px-4 pb-safe">
      ...
      <div className="relative z-10 text-center py-12 text-[10px] text-[#b0b0cc] opacity-40 font-mono tracking-widest leading-relaxed">
